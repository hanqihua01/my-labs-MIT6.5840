...
### Log compaction
Raft使用snapshot进行日志压缩，快照中包含了状态机的状态，以及使状态机达到此状态的、已提交的log entry的term和index，记为last included term和last included index。记录快照之后，在last included term和last included index及之前的log entry都可以删除，之前记录的快照也可以删除。

为了应对follower过于滞后或有新的server加入的情形，leader需要向其他server发送它的快照，这通过InstallSnapshot rpc完成。当follower收到server的快照时，如果该快照包含了自身没有的信息，那么就抛弃自身的所有log，根据快照内容重设状态机；否则，如果该快照是自身log的前缀，那么就删除被快照覆盖的log entry，后面的log entry仍然有效。

有两个影响记录快照性能的因素：第一是什么时候记录快照，一个简单的方式是当log entry达到一定大小时记录快照；第二是记录快照比较费时间，可以通过写时拷贝来解决，即向log添加entry的同时记录快照，而不是先记录快照，之后再向log添加entry。
### Client interaction
客户只能向leader发请求，当客户初次启动时，会随机向一个server发请求，如果该server不是leader，会拒绝客户请求，并提供leader的网络位置信息，之后客户就可以向leader发请求，如果leader崩掉了，客户请求会超时，然后客户会再次随机向一个server发请求。

目前为止，Raft对于同一个请求可能会执行多次，例如，leader已经commit某个请求之后挂掉了，客户没有收到回复则会再次请求，该请求可能被再次commit，解决办法是客户给每个请求加上一个序列号，这样如果客户再次发送同一个序列号的请求，状态机会追踪同一个用户最近提交的请求的序列号，发现两者相同，就直接回复客户，而不会再次执行该请求。
